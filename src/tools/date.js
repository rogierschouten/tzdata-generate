...
(function () {
  ...
  timezoneJS.timezone = new function () {
    ...
    function builtInLoadZoneFile(fileName, opts) {
      ...
      ? _this.parseZones(_this.transport({ url : url, async : false }), fileName)
      : _this.transport({
        async: true,
        url : url,
    ...
    function parseTimeString(str) {
	  ...
	  if (!hms) {
	    throw new Error("cannot parse time string (hms null)" + str);
	  }
	  if (hms.length < 4) {
	    throw new Error("cannot parse time string " + str);
	  }
	  if (hms[1] === null) {
	    throw new Error("cannot parse time string (1=null)" + str);
	  }
    ...
    function processZone(z) {
	  ...
	  try {
		var t = z[6] ? parseTimeString(z[6]) : [0, 0, 0];
			...
			throw new Error("error parsing \"" + z.toString() + "\": " + e.message);
		}
    ...
    this.parseZones = function (str, file) {
      var lines = str.split('\n')
        , arr = []
        , chunk = ''
        , l
          ...
          switch (chunk) {
            case 'Zone':
			  ...
			  try {
				arr[0] = -getBasicOffset(arr[0]);
					...
					throw new Error("Zone: error parsing file \"" + file + "\" line " + i.toString(10) + ": \"" + arr.toString() + "\": " + e.message);
				}
            ...
            case 'Rule':
			  ...
			  try {
					...
					throw new Error("Rule: error parsing \"" + arr.toString() + "\": " + e.message);
				}
              //Parse offset SAVE
